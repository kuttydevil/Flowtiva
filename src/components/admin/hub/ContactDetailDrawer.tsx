...
import { supabase, MESSAGE_PAGE_SIZE, whatsAppMessageFromSupabase } from '../../../services/supabaseService';
...
export const ContactDetailDrawer: React.FC<ContactDetailDrawerProps> = ({
    ...
    useEffect(() => {
        ...
        const fetchInitialMessages = async () => {
            ...
            try {
                const initialMessages = await supabase.getWhatsAppMessages(instance.id, contact.contact_name, 1);
        ...
        const channel = supabase.channel(`drawer-chat-${instance.id}-${contact.contact_name}`);
        channel.on('postgres_changes', 
            ...
            (payload) => {
                ...
                if (newMessage.contactName !== contact.contact_name) return;
    ...
    const handleLoadMoreMessages = async () => {
        ...
        try {
            const olderMessages = await supabase.getWhatsAppMessages(instance.id, contact.contact_name, nextPage);
            setMessages(prev => {
                const existingIds = new Set(prev.map(m => m.id));
                const uniqueOlderMessages = olderMessages.filter(m => !existingIds.has(m.id));
                
                if (uniqueOlderMessages.length === 0) {
                    if (olderMessages.length < MESSAGE_PAGE_SIZE) setHasMoreMessages(false);
                    return prev;
                }
    ...
    const handleSaveSuggestedTags = async (tagsToSave: string[]) => {
        ...
        try {
            await supabase.updateContactTags(instance.id, contact.contact_name, tagsToSave);
