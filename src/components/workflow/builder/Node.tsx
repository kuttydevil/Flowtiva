...
const useNodeDescription = (node: CanvasNodeData) => {
    const { t } = useTranslation();
    
    if (node.type === 'trigger') {
        const trigger = node.data as WorkflowTrigger;
        switch (trigger.type) {
            case 'new_contact_message': return { title: t('workflows.nodes.newContact.title'), details: t('workflows.nodes.newContact.details') };
            case 'crm_stage_changed': return { title: t('workflows.nodes.stageChanged.title'), details: trigger.config.stage ? t('workflows.nodes.stageChanged.details', { stage: trigger.config.stage }) : t('workflows.nodes.stageChanged.detailsEmpty') };
            case 'tag_added': return { title: t('workflows.nodes.tagAdded.title'), details: trigger.config.tag ? t('workflows.nodes.tagAdded.details', { tag: trigger.config.tag }) : t('workflows.nodes.tagAdded.detailsEmpty') };
            default: return { title: t('workflows.nodes.trigger.title'), details: t('workflows.nodes.trigger.details') };
        }
    } else {
        const action = node.data as WorkflowAction;
        switch (action.type) {
            case 'send_whatsapp_message': return { title: t('workflows.nodes.sendMessage.title'), details: action.config.message ? t('workflows.nodes.sendMessage.details', { message: action.config.message.substring(0, 40) }) : t('workflows.nodes.sendMessage.detailsEmpty') };
            case 'add_tag': return { title: t('workflows.nodes.addTag.title'), details: action.config.tag ? t('workflows.nodes.addTag.details', { tag: action.config.tag }) : t('workflows.nodes.addTag.detailsEmpty') };
            case 'change_crm_stage': return { title: t('workflows.nodes.changeStage.title'), details: action.config.stage ? t('workflows.nodes.changeStage.details', { stage: action.config.stage }) : t('workflows.nodes.changeStage.detailsEmpty') };
            case 'wait': return { title: t('workflows.nodes.wait.title'), details: t('workflows.nodes.wait.details', { days: action.config.days || 1 }) };
            default: return { title: t('workflows.nodes.action.title'), details: t('workflows.nodes.action.details') };
        }
    }
}
...
export const Node: React.FC<NodeProps> = ({ node, isSelected, isInvalid, onSelect, onDragStart }) => {
    const desc = useNodeDescription(node);
    ...
    return (
        ...
        >
            ...
            <div className="p-3 text-sm text-brand-text-secondary min-h-[48px]">
                {desc.details || <span className="italic opacity-70">{useTranslation().t('workflows.nodes.common.configure')}</span>}
            </div>
